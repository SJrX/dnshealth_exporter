...
- [X] T005 Create `demo/dashboard/main.go` (built in its final multi-variant shape from the start to avoid a refactor at T011, which became a no-op): a `main()` that loops over both variants (`dnshealth-overview` full + `dnshealth-overview-clean` no-info), calling `buildOverview(uid, title, includeInfoText)` for each, marshalling via `json.MarshalIndent` with a trailing newline, and writing via `os.WriteFile` to `demo/grafana/dashboards/dnshealth-overview{,-clean}.json`. Errors → `panic(err)` (idiomatic for a code generator; per plan.md Constitution table, Principle IV note). Stub `buildOverview(uid, title string, includeInfoText bool) (dashboard.Dashboard, error)` in `demo/dashboard/dashboard.go` that for now returns a minimal `dashboard.NewDashboardBuilder(title).Uid(uid).Build()` — just enough to validate the round-trip compiles and writes both files. After this task, `make dashboards` MUST succeed and produce two (mostly empty but valid and distinct) JSON files.
...
- [X] T023 Verified: `go build ./...` clean. `go list -deps .` confirms exporter binary does NOT link the SDK (Go strips unused deps). The exporter binary MUST build cleanly (Principle III, Principle VII). Confirm the SDK dep is in `go.mod` but the exporter binary size has not materially changed (Go strips unused deps at link time). Optional confirmation: `go list -deps ./` should NOT include `grafana-foundation-sdk` (the exporter's `main` does not import it).
- [X] T024 Both clean: `go vet ./...` ✓; all integration tests (cycle, dashboard, prober, etc.) green. Both MUST pass with zero new warnings or failures (constitution: Development Workflow).
- [X] T025 Updated `README.md` (repo root) "Try the demo" section with explicit link to `demo/README.md#iterate-on-the-dashboard-typed-go-source`. to add a one-line link from the existing demo section to the regen command (e.g., "Maintainers: regenerate the dashboards via `make dashboards` — see [demo/README.md](demo/README.md#dashboard-maintenance) for details.") if no equivalent link exists.
- [X] T026 Audit at `specs/005-dashboard-go-sdk/audit.md` — 17/17 FRs satisfied (FR-011 partial cosmetic drift), 8/8 SCs satisfied (SC-008 structural pending user browser-check), 6 deviations documented, 0 constitution violations. walk every FR (FR-001 through FR-017) and SC (SC-001 through SC-008) in `spec.md` and confirm the implementation satisfies it. Record any deviations in a new `specs/005-dashboard-go-sdk/audit.md` with file:line citations. Per constitution Governance: "After implementation, a thorough code audit against the spec MUST be performed before declaring the feature complete."
