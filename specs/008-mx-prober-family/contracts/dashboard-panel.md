...
| B | "All MX targets resolve" | `(dnshealth_mx_count{zone="$zone"} == bool 0) or on(zone) (dnshealth_mx_null_mx{zone="$zone"} == bool 1) or on(zone) ((dnshealth_mx_count{zone="$zone"} == bool dnshealth_mx_resolved_count{zone="$zone"}) and on(zone) (dnshealth_mx_count{zone="$zone"} > bool 0)) or on() vector(0)` | Metric: count comparison `_count` vs `_resolved_count`, with explicit pass-by-default for two cases: (a) zone has no MX records at all (count==0 — row A surfaces that case), (b) zone is Null MX (null_mx==1 — no targets to resolve). Why FAIL: at least one MX target's hostname does not resolve to A or AAAA. Inbound mail attempts for that target will fail. Investigate via the per-MX table below — `resolves=0` column. |
...
| D | "All MX target hostnames syntactically valid (LDH)" | `(dnshealth_mx_count{zone="$zone"} == bool 0) or on(zone) (min by (zone) (dnshealth_mx_syntax_valid{zone="$zone"}) == bool 1) or on() vector(0)` | Metric: `min(dnshealth_mx_syntax_valid) == 1`, with vacuous PASS when no MX records exist for the zone (count==0). Why FAIL: at least one MX target hostname has invalid syntax (underscore, leading/trailing hyphen, etc.) per RFC 952/1123. Some strict resolvers may reject. Reuses the same LDH check applied to NS hostnames in spec N6. |
