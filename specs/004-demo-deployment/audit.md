...
7. **Phase 9 records-table split + per-zone operator panels** — Phase 8's unified "NS records — parent vs self" table (with a `Source` column) conflated two viewpoints and made the per-zone story hard to read. User asked for two separate panels: one for parent's view (simple Nameserver / Glue IP) and one for the zone's self-reported view augmented with Responded and Recursion columns. Panel 6 became an outer-join of three queries (`ns_record{source="self"}`, `query_success{check="soa"}`, `ns_recursion_available`) by `nameserver`, filtered to rows where the self-side metric is present. For NSs only present on the self side (e.g. `ns-mismatch.demo.`'s `ns-internal-a/b`), Responded/Recursion cells are intentionally empty — the exporter only probes parent-listed NSs. The two operator-section time-series panels with per-zone interpretation ("SOA serials over time", "Query duration") now also filter via `{zone="$zone"}` and interpolate `${zone}` in their titles. See tasks T065–T068.

8. **Phase 10 code-review follow-ups** — small fixes from a post-Phase-9 adversarial code review (B1, B2, C1, C2, C3, C4, T1). Most consequential: `prober.RootServers` was initialized via slice-header copy of `DefaultRootServers`, sharing the same backing array; safe under current usage (every reassignment replaces the whole slice) but a footgun for future code that might do `RootServers[i] = X`. Switched to `append([]string(nil), DefaultRootServers...)` at init and in the reload restore-defaults path. Also rewrote stale Corefile / zone-file / Compose comments that lagged the Phase 8 renames, removed unused Compose aliases, set an explicit `$zone` default on the dashboard, tightened the smoke.sh exit-code 3 description, and added `TestStartup_WiresRootServersFromConfig` to close a unit-test coverage asymmetry between startup and reload code paths. See tasks T069–T076.
